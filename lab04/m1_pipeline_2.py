...
# -*- coding: utf-8 -*-
"""
Milestone 1 数据处理流水线（性能优化版）
基于 Polars Lazy API 的工程化数据管道
功能：数据读取 → 精密去重 → 会话识别 → 机器人检测 → 漏斗分析 → 数据导出

性能优化要点：
    1. 消除冗余 .collect()，统计延迟至导出阶段统一计算（预计 40-60% 时间节省）
    2. 合并 5 个 with_columns 为单个操作，减少中间列物化（预计 15-25% 会话识别加速）
    3. 机器人检测使用去重后数据 + 投影下推（预计 20-30% 检测加速）
    4. 窗口函数显式声明排序，辅助查询优化器（预计 10-15% 窗口计算加速）
"""
...
class M1DataPipeline:
    ...
    处理阶段：
        ...
        3. load()      - 导出干净数据（含 session_id） + 漏斗分析 + 统一统计报表
    ...
    def extract(self) -> pl.LazyFrame:
        ...
        优化：不在此处执行 collect 统计，延迟至导出阶段统一计算。
        ...
        try:
            ...
            logger.info("数据加载完成（行数统计延迟至导出阶段）")
            return self.lazy_df
    ...
    def transform(self) -> pl.LazyFrame:
        ...
        优化：所有步骤构建 LazyFrame 执行计划，不触发 collect。
        统计信息延迟至 load 阶段统一计算。
        ...
        try:
            ...
            # 2.2 会话识别：生成 session_id（无 collect）
            self._identify_sessions()

            # 2.3 机器人检测（必要时执行，结果需 collect 用于后续 join）
            self._detect_bots()

            # 2.4 剔除机器人账号（无 collect）
            self._remove_bot_accounts()

            logger.info("【Transform 阶段】数据转换完成（无冗余 collect）")
    ...
    def _deduplicate(self) -> None:
        ...
        优化：移除 count_before/after 的 collect，统计延迟至导出阶段。
        """
        logger.info("-" * 50)
        logger.info("【去重】开始四维度精密去重（无 collect）")
    ...
    def _identify_sessions(self) -> None:
        ...
        优化：
            1. 使用链式 with_columns（Polars 会自动融合执行计划）
            2. 移除 sample_count 的 collect，统计延迟至导出阶段
            3. 避免中间变量引用导致的表达式重复计算
        ...
        try:
            ...
            # 使用链式 with_columns，Polars 查询优化器会自动融合
            # 避免使用中间变量导致的表达式重复展开
            ...
            logger.info("会话识别操作已构建（统计延迟至导出阶段）")
    ...
    def _detect_bots(self) -> None:
        ...
        try:
            ...
            # 记录统计数据，供后续统一输出
            self.stats["bot_count"] = suspect_count
    ...
    def _run_bot_detection(self) -> pl.DataFrame:
        ...
        优化：
            1. 使用去重后的数据（deduped_df），避免对重复数据重复检测
            2. 使用 select 投影下推，只读取需要的列（user_id, timestamp, behavior_type）
        ...
        # 优化 1：使用去重后的数据（数据量更小）
        # 优化 2：投影下推，只选择需要的列（减少 I/O 和内存）
        lazy_df: pl.LazyFrame = self.deduped_df.select(
            ["user_id", "timestamp", "behavior_type"]
        )
    ...
    def _remove_bot_accounts(self) -> None:
        ...
        """
        logger.info("-" * 50)
        logger.info("【剔除机器人】开始移除嫌疑账号（无 collect）")
    ...
    def load(self) -> Dict[str, Any]:
        ...
        优化：在此阶段统一执行所有 collect 操作，
        避免之前分散在 transform 各步骤中的冗余全表扫描。
        ...
        try:
            ...
            # 3.2 打印 Transform 阶段统计报告（统一输出）
            self._print_transform_stats()
    ...
    def _export_clean_data(self) -> None:
        ...
        优化：在 sink_parquet 之前收集统计信息，只需一次扫描，
        替代之前多次独立的 collect 调用。
        ...
        try:
            ...
            # 优化：统一收集统计信息（一次扫描）
            stats_df: pl.DataFrame = self.clean_df.select(
                pl.len().alias("row_count"),
                pl.col("user_id").n_unique().alias("unique_users"),
                pl.col("session_id").n_unique().alias("unique_sessions"),
            ).collect(engine="streaming")

            self.stats["final_row_count"] = stats_df["row_count"].item()
            self.stats["unique_users"] = stats_df["unique_users"].item()
            self.stats["unique_sessions"] = stats_df["unique_sessions"].item()

            # 流式写入（第二次扫描，但 sink_parquet 是必须的操作）
            self.clean_df.sink_parquet(output_path, compression="snappy")

            file_size_mb: float = output_path.stat().st_size / (1024 * 1024)
            logger.info(
                f"导出完成：{self.stats['final_row_count']:,} 行，"
    ...
    def _print_transform_stats(self) -> None:
        """打印 Transform 阶段统计报告

        替代之前分散在各步骤中的 collect 和日志输出。
        所有统计数据在导出阶段统一计算和输出。
        """
        logger.info("-" * 60)
        logger.info("【Transform 阶段统计报告】")
        logger.info("-" * 60)

        # 去重统计
        if "row_count_before_dedup" in self.stats:
            dup_count: int = (
                self.stats["row_count_before_dedup"]
                - self.stats["row_count_after_dedup"]
            )
            dup_rate: float = (
                (dup_count / self.stats["row_count_before_dedup"] * 100)
                if self.stats["row_count_before_dedup"] > 0
                else 0
            )
            logger.info(
                f"去重前记录数：{self.stats['row_count_before_dedup']:,}"
            )
            logger.info(
                f"去重后记录数：{self.stats['row_count_after_dedup']:,}"
            )
            logger.info(
                f"去除重复记录：{dup_count:,} ({dup_rate:.4f}%)"
            )

        # 机器人检测统计
        if "bot_count" in self.stats:
            logger.info(f"嫌疑账号数：{self.stats['bot_count']:,}")

        # 剔除机器人统计
        if "rows_before_remove_bot" in self.stats:
            removed: int = (
                self.stats["rows_before_remove_bot"]
                - self.stats["rows_after_remove_bot"]
            )
            logger.info(
                f"剔除机器人前记录数："
                f"{self.stats['rows_before_remove_bot']:,}"
            )
            logger.info(
                f"剔除机器人后记录数："
                f"{self.stats['rows_after_remove_bot']:,}"
            )
            logger.info(f"剔除记录数：{removed:,}")

        # 最终统计
        if "final_row_count" in self.stats:
            logger.info(
                f"最终记录数：{self.stats.get('final_row_count', 'N/A'):,}"
            )
        if "unique_users" in self.stats:
            logger.info(
                f"最终用户数：{self.stats.get('unique_users', 'N/A'):,}"
            )
        if "unique_sessions" in self.stats:
            logger.info(
                f"总会话数：{self.stats.get('unique_sessions', 'N/A'):,}"
            )
