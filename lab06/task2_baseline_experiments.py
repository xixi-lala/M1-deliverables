...
# ==================== 7. 程序入口：任务2 - 批量运行6组基线实验 ====================
if __name__ == "__main__":
    # 定义6组实验参数 (实验组名, 生产速率λ, 单条处理耗时t, 消费者数n)
    experiments = [
        # A组：生产速率10条/秒
        ("A1", 10.0, 0.2, 1),
        ("A2", 10.0, 0.05, 1),
    ...
    # 清理旧的CSV文件，确保是全新的
    metrics_file = Path("experiment_metrics.csv")
    if metrics_file.exists():
        print(f"🧹 清理旧文件: {metrics_file}")
        metrics_file.unlink()
    
    print("=" * 60)
    print("🧪 任务2：开始运行6组基线实验 (无限队列，无背压)")
    print("=" * 60)
    
    # 批量运行所有实验
    for exp_name, producer_rate, consumer_time, n_consumers in experiments:
        print(f"\n\n{'='*60}")
        print(f"▶️  实验组: {exp_name}")
        print(f"   参数: λ={producer_rate}, t={consumer_time}, n={n_consumers}")
        print(f"{'='*60}")
        
        # 创建配置
        config = ExperimentConfig(
            producer_rate=producer_rate,
            consumer_process_time=consumer_time,
            n_consumers=n_consumers,
            queue_max_size=-1,  # 任务2：无限队列
            experiment_duration=15.0,
            enable_backpressure=False, # 任务2：不启用背压
    ...
    print("\n\n" + "=" * 60)
    print("✅ 所有6组基线实验完成！")
    print(f"📊 数据已保存至: {metrics_file.absolute()}")
    print("=" * 60)